...
const fetchBillboardsByLocation = async (location) => {
    try {
        ...
        console.log(`Advertisement4Urdu - Location ${location}: Found ${billboards.length} billboards`);
        return billboards;
    } catch (error) {
...
const Advertisement4Urdu = () => {
    const { data: billboards = [], isLoading, error } = useQuery({
        queryKey: ['billboards-urdu', 'location-8'],
        queryFn: () => fetchBillboardsByLocation(8),
        retry: false,
...
export default Advertisement4Urdu;
