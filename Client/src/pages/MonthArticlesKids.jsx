...
// Fetch articles for a specific month (Hilal Kids English only)
const fetchMonthArticlesKids = async (monthYear) => {
    try {
        ...
        params.append('publication', 'Hilal Kids English');
...
const MonthArticlesKids = () => {
    ...
    const { data: articles = [], isLoading, error } = useQuery({
        queryKey: ["monthArticlesKids", monthYear],
        queryFn: () => fetchMonthArticlesKids(monthYear),
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-4 mb-4">
                                <Link to="/hilal-kids" className="text-gray-500 hover:text-gray-700">
                                    <ChevronLeft className="w-5 h-5" />
                                </Link>
                                <h1 className="text-3xl font-bold text-gray-800">
                                    HILAL KIDS {displayDate}
                                </h1>
                            </div>
                            <p className="text-gray-600 mb-4">
                                Educational and entertaining content for children from {displayDate}.
                            </p>
                ...
                {articles.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500">
                            No kids articles found for {displayDate}
                        </p>
                    </div>
                ) : (
                    <>
                            ...
                            {currentArticles.map((article) => (
                                <div key={article.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden">
                                    ...
                                    <div className="p-4">
                                        ...
                                        <Link 
                                            to={`/article/${article.id}`}
                                            className="text-blue-600 hover:text-blue-700 font-medium text-sm transition-colors float-right"
                                        >
                                            Read More →
                                        </Link>
...
export default MonthArticlesKids;
