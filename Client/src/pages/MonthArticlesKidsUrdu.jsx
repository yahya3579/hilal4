...
// Fetch articles for a specific month (Hilal Kids Urdu only)
const fetchMonthArticlesKidsUrdu = async (monthYear) => {
    try {
        ...
        // Use filtered API to get articles for specific month/year and publication
...
const MonthArticlesKidsUrdu = () => {
    ...
    const { data: articles = [], isLoading, error } = useQuery({
        queryKey: ["monthArticlesKidsUrdu", monthYear],
        queryFn: () => fetchMonthArticlesKidsUrdu(monthYear),
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-4 mb-4">
                                <Link to="/hilal-urdu-kids" className="text-gray-500 hover:text-gray-700">
                                    <ChevronLeft className="w-5 h-5" />
                                </Link>
                                <h1 className="text-3xl font-bold text-gray-800 font-urdu-nastaliq-sm">
                                    ہلال بچوں کے لیے اردو {displayDate}
                                </h1>
                            </div>
                            <p className="text-gray-600 mb-4 font-urdu-nastaliq-sm">
                                {displayDate} کے بچوں کے مضامین جو تعلیمی اور تفریحی ہیں۔
                            </p>
                ...
                {articles.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 font-urdu-nastaliq-sm">
                            {displayDate} کے لیے کوئی بچوں کا مضمون نہیں ملا
                        </p>
                    </div>
                ) : (
                    <>
                                ...
                                <div key={article.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden">
                                    ...
                                    <div className="p-4">
                                        <Link 
                                            ...
                                            className="text-lg font-semibold text-gray-800 hover:text-green-600 transition-colors line-clamp-2 mb-2 block font-urdu-nastaliq-sm text-right"
                                        >
                                        ...
                                        {article.category_name && (
                                            <div className="mb-3 text-right">
                                                <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium font-urdu-nastaliq-sm">
                                                    {article.category_name}
                                                </span>
                                            </div>
                                        )}
                                        ...
                                        <Link 
                                            to={`/article/${article.id}`}
                                            className="text-green-600 hover:text-green-700 font-medium text-sm transition-colors float-left font-urdu-nastaliq-sm"
                                        >
                                            مزید پڑھیں ←
                                        </Link>
...
export default MonthArticlesKidsUrdu;
