...
// API calls
const fetchAuthors = async (page = 1, pageSize = 50, filters = {}) => {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/authors/?${params.toString()}`);
    return {
        data: res.data.data || [],
...
const AuthorsManagement = () => {
    ...
    const { data: queryData, isLoading, error } = useQuery({
        queryKey: ["authors", currentPage, pageSize, filters],
        queryFn: () => fetchAuthors(currentPage, pageSize, filters),
    });
    
    // Extract data and pagination info
    const authors = queryData?.data || [];
    ...
    const mutation = useMutation({
        ...
        onError: (error) => {
            showToast(`Error deleting author : ${error.response?.data || error.message}`, "error");
        },
    });
    ...
    // Get unique categories from current authors data
    const uniqueCategories = [...new Set((queryData?.data || []).map(author => author.category).filter(Boolean))].sort();
    ...
    return (
        <>
            ...
            <style>{`
                .authors-management-container::-webkit-scrollbar {
                    width: 8px;
                    height: 8px;
                }
                .authors-management-container::-webkit-scrollbar-track {
                    background: #f1f1f1;
                    border-radius: 10px;
                }
                .authors-management-container::-webkit-scrollbar-thumb {
                    background: #888;
                    border-radius: 10px;
                }
                .authors-management-container::-webkit-scrollbar-thumb:hover {
                    background: #555;
                }
                /* For Firefox */
                .authors-management-container {
                    scrollbar-width: thin;
                    scrollbar-color: #888 #f1f1f1;
                }
            `}</style>
            <div className="p-6 bg-white min-h-screen authors-management-container overflow-y-auto max-h-screen">
            {/* Header */}
            ...
            {pagination.total_pages > 0 && (
                <div className="mb-4 flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div className="text-sm text-gray-700 font-poppins">
                        Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, pagination.total_count || 0)} of {pagination.total_count || 0} authors
                    </div>
                </div>
            )}
            ...
            {pagination && pagination.total_pages > 0 && authors.length > 0 && (
