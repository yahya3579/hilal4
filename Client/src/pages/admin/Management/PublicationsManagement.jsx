...
// API calls
const fetchPublications = async (page = 1, pageSize = 50, filters = {}) => {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/publications/?${params.toString()}`);
    return {
        data: res.data.data || [],
...
const deletePublication = async (id) => {
    await axios.delete(`${import.meta.env.VITE_API_URL}/api/publication/${id}/`);
};
...
const PublicationsManagement = () => {
    ...
    const { data: queryData, isLoading, error } = useQuery({
        queryKey: ["publications", currentPage, pageSize, filters],
        queryFn: () => fetchPublications(currentPage, pageSize, filters),
    });
    
    // Extract data and pagination info
    const publications = queryData?.data || [];
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            if (publications.length === 1 && currentPage > 1) {
                setCurrentPage(currentPage - 1);
            }
        },
        onError: (error) => {
            showToast(`Error deleting publication: ${error.response?.data || error.message}`, "error");
        },
    });
    ...
    if (error) return <p>Error fetching publications</p>;

    return (
        <>
            {/* Custom Scrollbar Styles */}
            <style>{`
                .publications-management-container::-webkit-scrollbar {
                    width: 8px;
                    height: 8px;
                }
                .publications-management-container::-webkit-scrollbar-track {
                    background: #f1f1f1;
                    border-radius: 10px;
                }
                .publications-management-container::-webkit-scrollbar-thumb {
                    background: #888;
                    border-radius: 10px;
                }
                .publications-management-container::-webkit-scrollbar-thumb:hover {
                    background: #555;
                }
                /* For Firefox */
                .publications-management-container {
                    scrollbar-width: thin;
                    scrollbar-color: #888 #f1f1f1;
                }
            `}</style>
            <div className="p-6 bg-white min-h-screen publications-management-container overflow-y-auto max-h-screen">
            {/* Header */}
            <div className="relative mb-6">
                ...
                {/* New Publication Button - Top Left */}
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="mb-4">
                    ...
                    <input
                        ...
                        placeholder="Enter publication name or display name to search..."
            ...
            {pagination.total_pages > 0 && (
                <div className="mb-4 flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div className="text-sm text-gray-700 font-poppins">
                        Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, pagination.total_count || 0)} of {pagination.total_count || 0} publications
                    </div>
                </div>
            )}
            ...
            <div className="overflow-x-auto">
                {(!publications || publications.length === 0) ? (
                    <p className="text-center text-gray-500 font-poppins text-lg">No publications found.</p>
                ) : (
                                ...
                                >
                                    ...
                                    <td className="py-4 px-4 text-gray-700">
                                        {publication.cover_image ? (
                                            ...
                                            <div className="w-[120px] h-[47px] bg-gray-200 rounded flex items-center justify-center">
                                                <span className="text-gray-400 text-xs">No Image</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="py-4 px-4 text-gray-700">
                                        <span className="font-medium text-[12.7px] font-poppins">{publication.name}</span>
                                    </td>
                                    <td className="py-4 px-4 text-gray-700">
                                        <span className="font-medium text-[12.7px] font-poppins">{publication.display_name}</span>
                                    </td>
                                    <td className="py-4 px-4 text-gray-700">
                                        <span className="font-medium text-[12.7px] font-poppins">{publication.description || 'No description'}</span>
                                    </td>
                                    <td className="py-4 px-4 text-gray-700">
                                        <button className={`px-4 py-1 rounded text-[10.89px] font-bold ${publication.status === "Active" ? "bg-[#31AB5A] text-white" : "bg-gray-400 text-white"}`}>
                                            {publication.status}
                                        </button>
                                    </td>
                                    <td className="py-4 px-4 text-gray-700">
                                        <select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (action === "edit") {
                                                    window.location.href = `/admin/publication/edit/${publication.id}`;
            ...
            {pagination && pagination.total_pages > 0 && publications.length > 0 && (
